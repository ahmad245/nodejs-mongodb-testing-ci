const bootcampRepository = require("../repositories/BootcampRepository");
...
module.exports.getById = async (req, res, next) => {
  const bootcamp = await bootcampRepository.findById(req.params.id).cache({key:req.params.id});
  if (!bootcamp) {
...
module.exports.post = async (req, res, next) => {
 ...
 const publishedBootcamp = await bootcampRepository.findOne({ user: req.user.id }).cache({key:req.user.id});
...
module.exports.put = async (req, res, next) => {
  ...
  let bootcamp=await bootcampRepository.findById(req.params.id);
 ...
 if (bootcamp.user.toString() !== req.user.id  && req.user.role !== 'admin') {
  return  res.status(403).json({ success: false,error:`User ${req.user.id} is not authorized to update this bootcamp` });
   ...
   bootcamp = await bootcampRepository.update(req.params.id, req.body);
...
module.exports.remove = async (req, res, next) => {
 ...
 if (bootcamp.user.toString() !== req.user.id  && req.user.role !== 'admin') {
  ...
  await bootcampRepository.remove(req.params.id);
